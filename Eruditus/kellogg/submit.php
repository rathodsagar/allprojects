...
$lead_id 	  = '';
if($cp_source=="valuedirect")
{
	$lead_id 	  = MD5(time());
	$thankyoupage = "thankyou-vd.html?vd_lid=".$lead_id."&email=".$email;
}//
	...
	<tr>
    	<td>
        	<table width='100%' cellpadding='10' cellspacing='0' border='0'>
            	<tr>
                	<td bgcolor='#f0f0f0'><img src='http://www.eruditus.com/emailer/inseademailer/logo.jpg' width='149' height='87' alt='Insead' />
                    </td>
              </tr>
			  <tr>
                  <td height='10'></td>
                ...
                <tr>
                  <td height='10'><img src='http://www.eruditus.com/emailer/insead_faculty/images/ranking_03.jpg' /></td>
                </tr>
                ...
                <tr>
                  <td><font face='Arial, Helvetica, sans-serif' size='2' color='#666666'>Thanks for your interest in <strong>INSEAD's Leadership Programme for Senior Indian Executives (ILPSE)</strong>.</font></td>
                </tr>
                <tr>
                ...
                <tr>
                  <td><font face='Arial, Helvetica, sans-serif' size='2' color='#666666'>Best Regards,<br />
                    The INSEAD ILPSE Team</font></td>
                </tr>
                <tr>
               	  <td align='right' style='border-top:solid 1px #cccccc;'><img src='http://www.eruditus.com/programmes/ilpsie/images/partner.jpg' alt='Program Partner: Eruditus Executive Education' width='313' height='60' border='0' />
                    </td>
              </tr>
          </table>
        </td>
...
{
	if(isset($campaign))
		$subject="Kellogg SMP Program Query - [".$campaign."] [".$lead_source."] [".$country."] - M";
	else
		$subject="Kellogg SMP Program Query - [".$lead_source."] [".$country."] - M";
}
else
{
	if(isset($campaign))
		$subject="Kellogg SMP Program Query - [".$campaign."] [".$lead_source."] [".$country."]";
	else
		$subject="Kellogg SMP Program Query - [".$lead_source."] [".$country."]";
}

	$con=mysqli_connect("eruditusdb.db.7053204.hostedresource.com","eruditusdb","Q1w2e3!@#","eruditusdb") or die("cannot connect");
	$query=mysqli_query($con,"INSERT INTO  kellog_smp(vd_leadid,salutation,firstname,lastname,company,designation,city,country,phone,email,workexp,attenddate,finance,subject,date,source,medium,term,content,campaign,matchtype,network,creative,keyword,placement,random,copy,adposition,url) VALUES ('".$lead_id."','".$salutation."','".$firstname."','".$lastname."','".$company."','".$designation."','".$city."','".$country."','".$phone."','".$email."','".$workexp."','".$attenddate."','".$finance."','".$subject."','".$date."','".$source."','".$medium."','".$term."','".$content."','".$campaign."','".$matchtype."','".$network."','".$creative."','".$keyword."','".$placement."','".$random."','".$copy."','".$adposition."','".$url."')");
	$from="Kellogg_smp@eruditus.com";
	$mailheaders ="From: $email\nContent-Type: text/html; charset=iso-8859-1";
	$to = "meera.raman@eruditus.com"; //ruchira.roy@eruditus.com,
	//$to = "cool.jigs@gmail.com,trilokchand.modi@gmail.com";

	if(strtolower($workexp) == '< 10 years' && $cp_source=="valuedirect")
	{
		header('Location: http://www.eruditus.com/programmes/kellogg-SMP/'.$thankyoupage.'?vd_lid='.$lead_id.'&email='.$email.'&valid=0');
		exit;
	}
	else{
...
<html>
<head>
<title>Kellogg Senior Management Program (KSMP)</title>
...
<form class="form-horizontal mCustomScrollbar" role="form" id="frm" name="frm" action="https://www2.eruditus.com/l/63132/2017-05-10/zgxfyb" method="POST">
<input type="hidden" name="oid" value="00D90000000K0sz">
<input type="hidden" name="retURL" value="https://www.eruditus.com/programmes/kellogg-SMP/<?php echo $thankyoupage;?>">
<input type="hidden" name="00N90000002EM3X" value="Kellogg SMP" />
